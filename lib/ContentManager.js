...
module.exports = class ContentManager {
  ...
  generate() {
    ...
    return Promise.mapSeries(this.merchandise, mapFn)
      .then(() => {
        // generate a content manifest for each merchandise type
        types.forEach(type => {
          this.manifests[type] = this.merchandise
            .filter(merchandise => merchandise.getType() === type)
            .map(merchandise => merchandise.getFileName())
